...
public class OrderService {
    ...
    public Order placeOrder(OrderDTO orderDTO) {
        if (orderDTO == null
                || orderDTO.customerName() == null
                || orderDTO.productName() == null){
            throw new InvalidOrderException("Customer name and product name are required");
        }
        logger.info("Placing new order for customer '{}' - product '{}'", orderDTO.customerName(), orderDTO.productName());
    ...
    public Order createOrderWithCustomerDetailsFromApi(OrderDTO orderDTO) {
        Customer[] customersMatchingName = restClient.get().
                uri("/users?name=" + orderDTO.customerName()).
                retrieve().
                body(Customer[].class);

        if (customersMatchingName.length == 0){
            throw new CustomerNotFoundException(orderDTO.customerName());
        }
